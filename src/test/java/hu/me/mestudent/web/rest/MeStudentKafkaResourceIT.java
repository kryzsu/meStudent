...
import hu.me.mestudent.config.KafkaProperties;
...
class MeStudentKafkaResourceIT {
    ...
    void setup() {
        ...
        MeStudentKafkaResource kafkaResource = new MeStudentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/me-student-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/me-student-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
