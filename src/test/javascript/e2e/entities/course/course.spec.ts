...
describe('Course e2e test', () => {
  ...
  it('should load Courses', async () => {
    ...
    expect(await courseComponentsPage.getTitle()).to.eq('meStudentApp.course.home.title');
  ...
  it('should load create Course page', async () => {
    ...
    expect(await courseUpdatePage.getPageTitle()).to.eq('meStudentApp.course.home.createOrEditLabel');
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([courseUpdatePage.setNameInput('name'), courseUpdatePage.setCodeInput('code')]);

    expect(await courseUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await courseUpdatePage.getCodeInput()).to.eq('code', 'Expected Code value to be equals to code');
  ...
  it('should delete last Course', async () => {
    ...
    expect(await courseDeleteDialog.getDialogTitle()).to.eq('meStudentApp.course.delete.question');
